...
[흐름]
  ...
  2) 장중 최대 변동폭(peak)이 임계치를 넘으면 트리거(2단계, 하루 최대 2회):
       - 1차) |peak| >= THRESHOLD(4%): 원인 분석 풀 보고서 발송
       - 2차) |peak| >= THRESHOLD2(5%): 데이터 요약 속보 1회 추가 발송(1차 이후에만)
       - 1차 재료: OpenDART 당일 공시 + 네이버 뉴스 + KIS 지수/피어
  3) 위 재료를 Claude/Gemini에 보내 "원인 분석 보고서" 생성 (바이오 섹터 맥락 반영)
  4) 텔레그램 봇으로 지정 방에 전송
  5) 하루 중복 알림 방지 — 상태 파일에 '발송한 단계(tiers)'를 기록(최대 1·2차 2회)
...
def build_report(price_info, market, peers, narrative, news, direction, peer_news=None):
...
def build_summary_report(price_info, market, peers):
    """2차(추가 급변동) 속보 — 인사말·원인분석·뉴스 없이 핵심 수치 블록만.

    레이아웃: 변동폭 줄 + 현재/고가/저가 줄 / 지수 3종 / 피어 등락(수치만, 기사 링크 없음).
    1차 풀 보고서와 달리 피어엔 기사 링크를 붙이지 않는다(요청 형식 — 데이터만).
    """
    price, rate = price_info["price"], price_info["change_rate"]
    peak = price_info.get("peak_rate", rate)
    hi, lo = price_info.get("high_rate", rate), price_info.get("low_rate", rate)
    direction = _move_word(peak)
    pharma = market.get("pharma")
    pharma_line = f"{pharma['rate']:+.2f}% {_move_word(pharma['rate'])}" if pharma else "조회불가"
...
def analyze(price_info, market, peers, disclosures, news, news_ctx):
    ...
    return build_report(price_info, market, peers, narrative, news, direction, peer_news)
...
def alerted_tiers_today():
    """오늘 이미 발송한 알림 단계 집합({1}, {1,2} 등). 날짜가 다르거나 파일 없으면 빈 set."""
    if not os.path.exists(STATE_FILE):
        return set()
    try:
        with open(STATE_FILE) as f:
            d = json.load(f)
        if d.get("date") != str(datetime.date.today()):
            return set()
        return set(d.get("tiers", []))
    except Exception:
        return set()


def mark_alerted(tier):
    """오늘 날짜로 tier(1 또는 2) 발송을 기록한다(기존 단계 보존)."""
    tiers = alerted_tiers_today()
    tiers.add(tier)
    with open(STATE_FILE, "w") as f:
        json.dump({"date": str(datetime.date.today()), "tiers": sorted(tiers)}, f)
...
def main():
    ...
    # 발송 단계 결정 (하루 최대 2회: 1차 풀 보고서 → 2차 요약 속보).
    sent = alerted_tiers_today()
    if 1 not in sent:
        tier = 1                              # 첫 트리거(±4%↑): 풀 보고서
    elif abs_peak >= THRESHOLD2 and 2 not in sent:
        tier = 2                              # 1차 이후 ±5%↑로 심화: 요약 속보 추가 1회
    else:
        logging.info("오늘 발송 조건 미충족 — 스킵 (이미 발송=%s, peak=%+.2f%%)",
                     sorted(sent) or "없음", p["peak_rate"])
        return
    ...
    try:
        ...
        if tier == 1:
            ...
            news = get_related_news()         # 📰 관련 뉴스 푸터용(당사 우선 + 섹터 보충, 노이즈 제외)
            ...
            report = build_summary_report(p, market, peers)   # 2차: 데이터만(피어 링크 없음)
        send_telegram(report, parse_mode="HTML")
        mark_alerted(tier)
        logging.info("✅ 텔레그램 보고 완료 (%d차, peak %+.2f%%)", tier, p["peak_rate"])
    except Exception as e:
        logging.exception("보고 단계 실패 — 트리거됐으나 전송하지 못함")
        alert_admin(f"{tier}차 트리거(peak {p['peak_rate']:+.2f}%)됐으나 보고 실패: {type(e).__name__}: {e}")
        raise
