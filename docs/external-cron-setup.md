...
- **Body**: `{"ref":"main"}`
- **Schedule**: 타임존 `Asia/Seoul`, 요일 월–금, 시 9–20, 분 `*/5` (매 5분)
  - 20:00 컷오프는 스크립트 가드가 처리하므로 시 범위는 9–20으로 넉넉히 둬도 됨.
  - ⚠️ 2026-09-14 KRX 거래시간 연장(09:00~20:00)에 맞춰 cron-job.org 작업의 시 범위를 **9–20으로 직접 수정**해야 함(사이트 설정은 코드로 못 바꿈).
