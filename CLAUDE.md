...
- `ANTHROPIC_API_KEY` — Claude API. **미설정/비어도 동작**(기본 보고서 폴백 모드).
...
- **TODO #5 일부** — 뉴스를 클릭 가능한 하이퍼링크로 발송.

## 남은 작업 (TODO — 우선순위 순)
1. **검수용 라우팅**(구 #2): 현재는 보고 방에 직접 전송한다. AI 원인 분석은 추정이므로,
   사장님 방 직행 전 IR 담당자 검수 단계(담당자 방 선발송 → 승인 시 전달)를 넣는 옵션을 고려.
2. **뉴스 단계 보강 나머지**(구 #5): 검색어 다양화(종목명+이슈), 중복 제거, 시각 필터 등 정확도 개선.
3. (선택) **n8n 포팅**(구 #6): 노코드 유지보수가 필요하면 동일 흐름을 n8n 워크플로로 옮긴다.
