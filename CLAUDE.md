...
- 하루 1회 보고(±5% 풀 보고서). `~/.stock_alert_302440_state.json`에 당일 보고 날짜를 기록해 중복 발송을 막는다.
...
- **상태 파일 형식**: `~/.stock_alert_302440_state.json` = `{"date": "YYYY-MM-DD"}`. 날짜가 오늘과 같으면 이미 보고한 것으로 간주(하루 1회).
- **텔레그램은 HTML 모드 전송**(`send_telegram(parse_mode="HTML")`): 뉴스 제목을 `<a>` 하이퍼링크로 발송하고
  모든 동적 텍스트를 `html.escape` 처리. '관련 뉴스'는 본 보고서에 붙이지 않고 별도 메시지로 발송한다.
